...
- Remotes: `origin` is the domain repo; `harness-core` is `ai-harness-core`. The scaffolder does not push.
...
`python scripts/sync/pull_harness_core.py --dry-run --json` fetches `harness-core` and copies **allowlisted core paths only** onto a new branch (`chore/pull-harness-core-<utc>`). Domain overlay files are skipped. The command never merges into the previous branch and never pushes.
